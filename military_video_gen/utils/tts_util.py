            ...
            try:
                ...
                if output_path:
                    with open(output_path, "wb") as f:
                        f.write(audio_data)
                    logger.info(f"Audio saved to: {redact_path_for_log(output_path)}")
                ...
                if error_code == 401 or '401' in error_msg:
                    ...
                    logger.debug(f"Error details: {sanitize_error_message(error_msg)}")
                    ...
                    logger.warning(
                        ...
                        f"{error_code} - {sanitize_error_message(e)}"
                    )
        ...
        for attempt in range(retry_count + 1):
            ...
            try:
                ...
                if error_code == 401 or '401' in error_msg:
                    ...
                    logger.warning(
                        f"⚠️  List voices error (attempt {attempt + 1}/{retry_count + 1}): "
                        f"{error_code} - {sanitize_error_message(e)}"
                    )
                ...
                # Other errors - don't retry, raise immediately
                logger.error(
                    "List voices error (non-retryable): "
                    f"{type(e).__name__} - {sanitize_error_message(e)}"
                )
                raise
