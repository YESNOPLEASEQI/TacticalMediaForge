...
Provides utilities for managing paths and files in MilitaryVideoGen.
"""
...
def get_military_video_gen_root_path() -> str:
    """
    Get MilitaryVideoGen root path
    
    Uses MILITARY_VIDEO_GEN_ROOT environment variable to determine project root.
    ...
    env_root = os.environ.get("MILITARY_VIDEO_GEN_ROOT")
...
def ensure_military_video_gen_root_path() -> str:
    """
    Ensure MilitaryVideoGen root path exists and return the path
    
    Returns:
        Root path as string
    """
    root_path = get_military_video_gen_root_path()
...
def get_root_path(*paths: str) -> str:
    """
    Get path relative to MilitaryVideoGen root path
    ...
    """
    root_path = ensure_military_video_gen_root_path()
    if paths:
...
def get_temp_path(*paths: str) -> str:
    """
    Get path relative to MilitaryVideoGen temp folder
...
def get_data_path(*paths: str) -> str:
    """
    Get path relative to MilitaryVideoGen data folder
...
def get_output_path(*paths: str) -> str:
    """
    Get path relative to MilitaryVideoGen output folder
...
def _validate_resource_paths(paths: tuple[str, ...]) -> None:
    """Reject absolute and parent-relative resource names before path joins."""
    if not paths:
        raise ValueError("resource path must name a file")
    for raw_part in paths:
        if not isinstance(raw_part, str) or not raw_part.strip() or "\x00" in raw_part:
            raise ValueError("resource path contains an invalid component")
        part = Path(raw_part)
        if part.is_absolute() or part.drive or any(piece in {"", ".", ".."} for piece in part.parts):
            raise ValueError("resource path must stay inside its resource directory")


def _contained_resource_path(base: str, *paths: str) -> str:
    root = Path(base).resolve()
    candidate = root.joinpath(*paths).resolve()
    try:
        candidate.relative_to(root)
    except ValueError as exc:
        raise ValueError("resource path resolves outside its resource directory") from exc
...
def list_resource_files(
    ...
    """
    if subdir:
        _validate_resource_paths((subdir,))
