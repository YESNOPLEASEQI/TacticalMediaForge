...
Example:
    pipeline = AssetBasedPipeline(military_video_gen)
...
from military_video_gen.utils.os_util import (
...
class AssetBasedPipeline(LinearVideoPipeline):
    ...
    def __init__(self, core):
        ...
        Args:
            core: MilitaryVideoGenCore instance
        """
        super().__init__(core)
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        enforce_safe_generation_fields(intent=intent, video_title=video_title)
    ...
    async def setup_environment(self, context: PipelineContext) -> PipelineContext:
        ...
        for i, asset_path in enumerate(assets, 1):
            ...
            if not asset_path_obj.exists():
                logger.warning(f"Asset not found: {redact_path_for_log(asset_path)}")
                continue
            ...
            if asset_type == "image":
                ...
                try:
                    ...
                    logger.warning(
                        "Video analysis failed for "
                        f"{asset_path_obj.name}: {sanitize_error_message(e)}, using fallback"
                    )
                    self.asset_index[asset_path] = {
                        "path": asset_path,
                        "type": "video",
                ...
                logger.warning(f"Unknown asset type: {redact_path_for_log(asset_path)}")
    ...
    async def determine_title(self, context: PipelineContext) -> PipelineContext:
        ...
        if title:
            context.title = title
            logger.info("📝 Using user-specified video title")
        else:
            context.title = ""
    ...
    async def generate_content(self, context: PipelineContext) -> PipelineContext:
        ...
        """
        from military_video_gen.prompts.asset_script_generation import build_asset_script_prompt
        ...
        enforce_safe_generation_fields(
            generated_narrations=[
                ...
                for narration in scene.get("narrations", [])
            ]
        )
                ...
                for known_path in self.asset_index.keys():
                    if Path(known_path).name == Path(asset_path).name:
                        ...
                        logger.warning(
                            ...
                            f"{redact_path_for_log(asset_path)} -> "
                            f"{redact_path_for_log(known_path)}"
                        )
                        break
        ...
        for scene in context.script:
            ...
            logger.info(f"Scene {scene['scene_number']} [{asset_name}] prepared")
    ...
    async def produce_assets(self, context: PipelineContext) -> PipelineContext:
        ...
        for i, frame in enumerate(storyboard.frames, 1):
            ...
            for j, narration_text in enumerate(narrations, 1):
                enforce_safe_generation_fields(narration=narration_text)
                ...
                logger.debug(f"  Narration {j}/{len(narrations)} prepared")
            ...
            if len(narration_audios) > 1:
                ...
                await run_blocking_until_stopped(self._video_service._run_command, concat_cmd)
            ...
            duration_result = await run_blocking_until_stopped(
                self._video_service._run_command,
                duration_cmd,
            )
            ...
            if api_video_workflow and frame.media_type == "image" and frame.image_path:
                ...
                tail_frame_path = (
                    Path(context.task_dir) / "frames" / f"{i:02d}_api_tail_reference.png"
                )
                extracted_tail = await self._extract_video_tail_frame(
                    frame.video_path,
                    str(tail_frame_path),
                )
                if extracted_tail:
                ...
                logger.success(
                    "✅ API video generated for scene "
                    f"{i}: {redact_path_for_log(frame.video_path)}"
                )
            ...
            if (
                api_video_workflow
                and not api_video_generated_for_frame
                and processed_frame.video_segment_path
            ):
    ...
    async def post_production(self, context: PipelineContext) -> PipelineContext:
        ...
        filename = safe_output_filename(
            context.title,
            suffix=".mp4",
            fallback=context.task_id or "final",
        )
        ...
        await run_blocking_until_stopped(
            self._video_service.concat_videos,
            videos=scene_videos,
        ...
        context.storyboard.total_duration = probe_media_file(
            final_video_path,
            require_audio=True,
        )

        logger.success(f"✅ Final video: {redact_path_for_log(final_video_path)}")
    ...
    async def finalize(self, context: PipelineContext) -> PipelineContext:
        ...
        logger.info(f"Video: {redact_path_for_log(context.final_video_path)}")
    ...
    def _get_api_workflow_info(self, workflow_key: str) -> dict:
        ...
        try:
            ...
            logger.warning(
                "Failed to read API workflow metadata for "
                f"{workflow_key}: {sanitize_error_message(exc)}"
            )
        return {}

    async def _extract_video_tail_frame(
        self,
        video_path: str,
        output_path: str,
    ) -> Optional[str]:
        ...
        try:
            ...
            await run_blocking_until_stopped(self._video_service._run_command, extract_cmd)
            if Path(output_path).exists():
                logger.info(
                    f"Extracted tail reference frame: {redact_path_for_log(output_path)}"
                )
                return output_path
        except Exception as exc:
            logger.debug(
                "Primary tail-frame extraction failed for "
                f"{redact_path_for_log(video_path)}: {sanitize_error_message(exc)}"
            )
        ...
        try:
            ...
            probe = await run_blocking_until_stopped(
                self._video_service._run_command,
                probe_cmd,
            )
            ...
            await run_blocking_until_stopped(self._video_service._run_command, fallback_cmd)
            if Path(output_path).exists():
                logger.info(
                    "Extracted tail reference frame with fallback: "
                    f"{redact_path_for_log(output_path)}"
                )
                return output_path
        except Exception as exc:
            logger.warning(
                "Failed to extract tail frame from "
                f"{redact_path_for_log(video_path)}: {sanitize_error_message(exc)}"
            )
        return None
