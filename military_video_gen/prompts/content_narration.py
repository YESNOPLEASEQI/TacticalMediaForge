...
- Chinese length: Each narration must be a compact 12~22 Chinese-character subtitle card
- Other languages: Strictly control to {min_words}~{max_words} words
- Subtitle rhythm: Put only one main clause or one compact question in each narration
- Chinese punctuation: Normally use no comma; use at most one comma only when essential, and never chain several facts with commas
- Concision: Move the next idea into the next storyboard instead of extending the current narration; remove filler, stacked modifiers, and repeated conclusions
...
- Refinement strategy:
  ...
  * If user content is short: Retain its core viewpoint without padding or unnecessary expansion
...
3. Chinese narrations must stay within 12~22 Chinese characters; other languages use {min_words}~{max_words} words
