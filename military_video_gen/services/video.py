...
class VideoService:
    ...
    def cancel_active_operations(self) -> None:
        """Terminate FFmpeg children owned by this service instance."""
        with self._active_process_lock:
    ...
    def _run_ffmpeg(self, graph) -> tuple[bytes, bytes]:
        """Run an ffmpeg-python graph while retaining a cancellable Popen."""
        process = graph.run_async(pipe_stdout=True, pipe_stderr=True)
    ...
    def _run_command(self, command: list[str]) -> subprocess.CompletedProcess[str]:
        """Run a raw FFmpeg command while retaining a cancellable Popen."""
    ...
    def concat_videos(
        ...
        if len(videos) == 1 and not bgm_path:
            logger.info(f"Only one video provided, copying to {redact_path_for_log(output)}")
        ...
        if bgm_path:
            ...
            temp_output = self._get_unique_temp_path("no_bgm", Path(output).name)
            try:
                if len(videos) == 1:
                    shutil.copy2(videos[0], temp_output)
                ...
                logger.info(
                    f"Adding BGM: {redact_path_for_log(bgm_path)} "
    ...
    def _concat_demuxer(self, videos: List[str], output: str) -> str:
        ...
        try:
            logger.debug(f"Created filelist: {redact_path_for_log(filelist)}")
            self._run_ffmpeg(
            ...
            logger.success(
                f"Videos concatenated successfully: {redact_path_for_log(output)}"
            )
            return output
        except ffmpeg.Error as e:
            error_msg = e.stderr.decode() if e.stderr else str(e)
            logger.error(f"FFmpeg concat error: {sanitize_error_message(error_msg)}")
            raise RuntimeError(
                f"Failed to concatenate videos: {sanitize_error_message(error_msg)}"
            )
        finally:
    ...
    def _concat_filter(self, videos: List[str], output: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"FFmpeg concat filter error: {sanitize_error_message(error_msg)}"
            )
            raise RuntimeError(
                f"Failed to concatenate videos: {sanitize_error_message(error_msg)}"
            )
        except Exception as e:
            logger.error(f"Concatenation error: {sanitize_error_message(e)}")
            raise RuntimeError(
                f"Failed to concatenate videos: {sanitize_error_message(e)}"
            ) from e
    ...
    def has_audio_stream(self, video: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Video {redact_path_for_log(video)} has_audio={has_audio}"
            )
            return has_audio
        except Exception as e:
            logger.warning(
                "Failed to probe video audio streams: "
                f"{sanitize_error_message(e)}, assuming no audio"
            )
            return False
    ...
    def merge_audio_video(
        ...
        adjusted_video: Optional[str] = None
        ...
        if auto_adjust_duration:
            ...
            if diff < 0:
                ...
                adjusted_video = video if video != original_video else None
        ...
        if not video_has_audio:
            ...
            try:
                ...
                logger.success(
                    f"Audio added to silent video: {redact_path_for_log(output)}"
                )
                if adjusted_video:
                    Path(adjusted_video).unlink(missing_ok=True)
                return output
            except ffmpeg.Error as e:
                if adjusted_video:
                    Path(adjusted_video).unlink(missing_ok=True)
                error_msg = e.stderr.decode() if e.stderr else str(e)
                logger.error(
                    "FFmpeg error adding audio to silent video: "
                    f"{sanitize_error_message(error_msg)}"
                )
                raise RuntimeError(
                    f"Failed to add audio to video: {sanitize_error_message(error_msg)}"
                )
        ...
        try:
            ...
            logger.success(f"Audio merged successfully: {redact_path_for_log(output)}")
            if adjusted_video:
                Path(adjusted_video).unlink(missing_ok=True)
            return output
        except ffmpeg.Error as e:
            if adjusted_video:
                Path(adjusted_video).unlink(missing_ok=True)
            error_msg = e.stderr.decode() if e.stderr else str(e)
            logger.error(f"FFmpeg merge error: {sanitize_error_message(error_msg)}")
            raise RuntimeError(
                f"Failed to merge audio and video: {sanitize_error_message(error_msg)}"
            )
    ...
    def overlay_image_on_video(
        ...
        try:
            ...
            self._run_ffmpeg(
                ffmpeg.output(output_stream, output,
                        vcodec='libx264',
                        pix_fmt='yuv420p',
                        preset='medium',
                        crf=23)
            ...
            logger.success(f"Image overlaid on video: {redact_path_for_log(output)}")
            return output
        except ffmpeg.Error as e:
            error_msg = e.stderr.decode() if e.stderr else str(e)
            logger.error(f"FFmpeg overlay error: {sanitize_error_message(error_msg)}")
            raise RuntimeError(
                f"Failed to overlay image on video: {sanitize_error_message(error_msg)}"
            )

    def overlay_timed_images_on_video(
        self,
        video: str,
        ...
        """Overlay a sequence of subtitle cards during selected time ranges."""
        self._ensure_ffmpeg()
        if not overlays:
            raise ValueError("Overlays list cannot be empty")

        logger.info(f"Overlaying {len(overlays)} timed images on video")
        try:
            overlay_probe = ffmpeg.probe(overlays[0][0])
            overlay_stream = next(
                stream
                for stream in overlay_probe["streams"]
                if stream["codec_type"] == "video"
            )
            width = int(overlay_stream["width"])
            height = int(overlay_stream["height"])
            input_video = ffmpeg.input(video)
            ...
            if scale_mode == "contain":
                ...
                output_stream = input_video.filter("scale", width, height)

            for image_path, start, end in overlays:
                overlay = ffmpeg.input(image_path, loop=1).video
            ...
            logger.success(
                f"Timed images overlaid on video: {redact_path_for_log(output)}"
            )
            return output
        except ffmpeg.Error as error:
            error_msg = error.stderr.decode() if error.stderr else str(error)
            logger.error(
                f"FFmpeg timed overlay error: {sanitize_error_message(error_msg)}"
            )
            raise RuntimeError(
                f"Failed to overlay timed images: {sanitize_error_message(error_msg)}"
            )
    ...
    def create_video_from_image(
        ...
        try:
            ...
            logger.success(
                f"Video created from image: {redact_path_for_log(output)} "
            ...
            error_msg = e.stderr.decode() if e.stderr else str(e)
            logger.error(
                "FFmpeg error creating video from image: "
                f"{sanitize_error_message(error_msg)}"
            )
            raise RuntimeError(
                f"Failed to create video from image: {sanitize_error_message(error_msg)}"
            )
    ...
    def create_video_from_timed_images(
        ...
        """Create one scene from subtitle-card images that change over its audio."""
        self._ensure_ffmpeg()
        if not images:
            raise ValueError("Images list cannot be empty")

        logger.info(f"Creating video from {len(images)} timed images and audio")
        try:
            video_streams = []
            for image_path, duration in images:
                stream = (
                    ffmpeg
                    .input(image_path, loop=1, framerate=fps, t=max(duration, 0.001))
                    .video
            ...
            logger.success(
                f"Video created from timed images: {redact_path_for_log(output)} "
                f"(duration: {audio_duration:.3f}s)"
            )
            return output
        except ffmpeg.Error as error:
            error_msg = error.stderr.decode() if error.stderr else str(error)
            logger.error(
                f"FFmpeg timed images error: {sanitize_error_message(error_msg)}"
            )
            raise RuntimeError(
                "Failed to create video from timed images: "
                f"{sanitize_error_message(error_msg)}"
            )
    ...
    def add_bgm(
        ...
        try:
            ...
            logger.success(f"BGM added successfully: {redact_path_for_log(output)}")
            return output
        except ffmpeg.Error as e:
            error_msg = e.stderr.decode() if e.stderr else str(e)
            logger.error(f"FFmpeg BGM error: {sanitize_error_message(error_msg)}")
            raise RuntimeError(
                f"Failed to add BGM: {sanitize_error_message(error_msg)}"
            )
    ...
    def _list_available_bgm(self) -> list[str]:
        ...
        try:
            ...
            logger.warning(f"Failed to list BGM files: {sanitize_error_message(e)}")
            return []
    ...
    def _trim_video_to_duration(self, video: str, target_duration: float) -> str:
        ...
        try:
            ...
            error_msg = e.stderr.decode() if e.stderr else str(e)
            logger.error(
                f"FFmpeg error trimming video: {sanitize_error_message(error_msg)}"
            )
            raise RuntimeError(
                f"Failed to trim video: {sanitize_error_message(error_msg)}"
            )
    ...
    def _pad_video_to_duration(self, video: str, target_duration: float, pad_strategy: str = "freeze") -> str:
        ...
        try:
            ...
            error_msg = e.stderr.decode() if e.stderr else str(e)
            logger.error(
                f"FFmpeg error padding video: {sanitize_error_message(error_msg)}"
            )
            raise RuntimeError(
                f"Failed to pad video: {sanitize_error_message(error_msg)}"
            )
