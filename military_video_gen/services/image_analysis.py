...
class ImageAnalysisService(ComfyBaseService):
    ...
    Usage:
        ...
        description = await military_video_gen.image_analysis("path/to/image.jpg")
        
        # Use local ComfyUI
        description = await military_video_gen.image_analysis(
            "path/to/image.jpg",
            source="selfhost"
        )
        
        # List available workflows
        workflows = military_video_gen.image_analysis.list_workflows()
    """
    ...
    async def __call__(
        ...
        **params
        ...
        Examples:
            ...
            description = await military_video_gen.image_analysis("temp/06.JPG")
        ...
        if not image_path_obj.exists():
            raise FileNotFoundError(
                f"Image file not found: {redact_path_for_log(image_path)}"
            )
        ...
        try:
            ...
            if result.status != "completed":
                ...
                logger.error(f"Image analysis failed: {sanitize_error_message(error_msg)}")
                raise RuntimeError(
                    f"Image analysis failed: {sanitize_error_message(error_msg)}"
                )
            ...
            if not description:
                logger.error(
                    f"No text found in output keys: {sorted(result.outputs or {})}"
                )
                raise Exception("No description generated")
            
            logger.info(f"Image analyzed successfully ({len(description)} chars)")
            
            return description
        
        except Exception as e:
            logger.error(f"Image analysis error: {sanitize_error_message(e)}")
            raise
