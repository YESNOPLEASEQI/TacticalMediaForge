...
class MediaService(ComfyBaseService):
    ...
    Usage:
        ...
        if media.is_image:
            print(f"Generated image: {redact_url_for_log(media.url)}")
        elif media.is_video:
            print(
                f"Generated video: {redact_url_for_log(media.url)} "
                f"({media.duration}s)"
            )
        
        # Use specific workflow
        media = await military_video_gen.media(
            prompt="a cat",
            workflow="image_flux.json"
        )
        
        # List available workflows
        workflows = military_video_gen.media.list_workflows()
    """
        ...
        for source_name in source_dirs:
            ...
            for filename in matching_files:
                try:
                    ...
                    logger.error(
                        f"Failed to parse workflow {source_name}/{filename}: "
    ...
    async def __call__(
        ...
        **params
        ...
        Examples:
            ...
            media = await military_video_gen.media(prompt="a beautiful cat")
            if media.is_image:
                print(f"Image: {redact_url_for_log(media.url)}")
            ...
            if media.is_video:
                print(
                    f"Video: {redact_url_for_log(media.url)}, "
                    f"duration: {media.duration}s"
                )
        ...
        try:
            ...
            if result.status != "completed":
                ...
                logger.error(
                    f"Media generation failed: {sanitize_error_message(error_msg)}"
                )
                raise RuntimeError(
                    f"Media generation failed: {sanitize_error_message(error_msg)}"
                )
            ...
            if media_type == "video":
                ...
                logger.info(f"✅ Generated video: {redact_url_for_log(video_url)}")
                ...
                logger.info(f"✅ Generated image: {redact_url_for_log(image_url)}")
