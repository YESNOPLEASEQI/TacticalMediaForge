...
class APIProviderMediaService:
    """Adapter from MilitaryVideoGen media calls to direct provider API clients."""
    ...
    VIDEO_MODEL_CAPABILITIES: dict[tuple[str, str], dict[str, Any]] = {
        ...
        ("dashscope", "wan2.7-i2v"): {
            ...
            "contract_issues": [
                "MilitaryVideoGen UI exposes first-frame image-to-video; asset-based workflow can optionally pass narration audio as driving_audio.",
    ...
    async def _generate_image(
        ...
        **params,
        ...
        session_id = params.get("session_id") or "military_video_gen"

        logger.info(f"Generating image via API provider={provider}, model={model}")
        paths = await run_blocking_until_stopped(
        ...
        validated_path, _ = validate_generated_output(result_path, media_kind="image")
        return MediaResult(media_type="image", url=str(validated_path))
        ...
        for attempt in range(max_safety_retries + 1):
            try:
                ...
                logger.warning(
                    ...
                    "neutralizing prompt and retrying once. "
                    f"provider={provider}, model={model}, "
                    f"error={sanitize_error_message(exc)}"
                )
                prompt_to_use = await self._neutralize_video_prompt(prompt_to_use)

        if not os.path.exists(save_path):
            raise RuntimeError(
                "API video generation did not create file: "
                f"{redact_path_for_log(save_path)}"
            )
...
""".strip()
        ...
        try:
            ...
            logger.warning(
                "Failed to neutralize video prompt with LLM; using fallback sanitizer: "
    ...
    def _create_image_client(self):
        from military_video_gen.services.api_services.image_client import ImageClient
    ...
    def _create_video_client(self):
        from military_video_gen.services.api_services.video_client import VideoClient
    ...
    def _video_options(
        ...
        """Map MilitaryVideoGen's generic video params to provider client options."""
