...
class DashscopeVideoClient:
    ...
    def _with_network_retry(self, action_name: str, func, max_attempts: int = 5, base_delay: float = 3.0):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            logger.warning(
                ...
                max_attempts,
                delay,
                sanitize_error_message(last_error),
            )
            time.sleep(delay)

        raise RuntimeError(
            f"DashScope {action_name} failed after {max_attempts} attempts due to network error: "
            f"{sanitize_error_message(last_error)}"
        ) from last_error

    def _submit_once(self, func):
        """Submit a provider task once because creation is not idempotent."""
        with self._proxy_env():
            return func()
    ...
    def generate_video(
        ...
        if self._is_text_to_video_model(model):
            ...
            rsp = self._submit_once(lambda: VideoSynthesis.call(**call_kwargs))
        ...
        logger.info(f"DashscopeVideoClient: video saved: {redact_path_for_log(save_path)}")
        return video_url

    def cancel_active_operations(self) -> None:
        """Cancel the active provider task when DashScope has returned an id."""
        with self._active_task_lock:
            task_id = self._active_task_id
        if not task_id or VideoSynthesis is None:
            return
        logger.info("DashscopeVideoClient: requesting cancellation for task_id=%s", task_id)
        self._with_network_retry(
            f"cancel task {task_id}",
            lambda: VideoSynthesis.cancel(task=task_id, api_key=self.api_key),
            max_attempts=3,
            base_delay=1.0,
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(IMAGE_PATH):
        print(f"✗ 输入图片不存在: {redact_path_for_log(IMAGE_PATH)}")
        sys.exit(1)
    ...
    for model in MODELS:
        ...
        print(f"  Base URL   : {redact_url_for_log(base_url)}")
        print(f"  输入图片   : {redact_path_for_log(IMAGE_PATH)}")
        print(f"  输出路径   : {redact_path_for_log(output_path)}")
        print(f"  模型       : {model}")
        ...
        try:
            ...
            print(f"  远端 URL : {redact_url_for_log(video_url)}")
            print(f"  本地文件 : {redact_path_for_log(output_path)}")
            print(f"  文件大小 : {os.path.getsize(output_path) / 1024 / 1024:.2f} MB")
        except Exception as e:
            print(f"✗ 失败: {sanitize_error_message(e)}")
            sys.exit(1)
