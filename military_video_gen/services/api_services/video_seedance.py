...
class SeedanceVideoClient:
    ...
    def _submit_task(self, prompt: str, image_path: Optional[str], model: str, duration: int, **kwargs) -> str:
        ...
        if not resp.ok:
            logger.error(f"Seedance submission failed: {sanitize_error_message(resp.text)}")
    ...
    def _poll_until_done(self, task_id: str, max_polls: int = 120, interval: int = 5) -> str:
        ...
        for i in range(max_polls):
            ...
            if status == "succeeded":
                ...
                raise RuntimeError(
                    f"Seedance 视频生成{status}: {sanitize_error_message(error_msg)}"
                )
    ...
    def _download_video(self, url: str, save_path: str):
        ...
        with open(save_path, "wb") as f:
            for chunk in resp.iter_content(chunk_size=8192):
                if chunk:
                    f.write(chunk)
        logger.info(f"SeedanceVideoClient: video saved: {redact_path_for_log(save_path)}")
