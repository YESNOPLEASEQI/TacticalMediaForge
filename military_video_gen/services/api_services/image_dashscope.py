    ...
    def generate_image(self, prompt, model="wan2.7-image", size="1024*1024", n=1, session_id=None, save_dir=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not results:
                    raise RuntimeError(
                        "DashScope image generation returned no image URLs"
                    )
            ...
            logging.error(
                f"Error in generate_image (DashScope): {sanitize_error_message(e)}"
            )
            raise
    ...
    def edit_image(self, prompt, image_urls, model="wan2.7-image", size="1920*1080", n=1, session_id=None, save_dir=None):
        ...
        try:
            ...
            if response.status_code == 200:
                results = self._extract_image_urls(getattr(response, "output", None))
                if not results:
                    raise RuntimeError("DashScope image edit returned no image URLs")
            ...
            logging.error(f"Error in edit_image: {sanitize_error_message(e)}")
            raise
