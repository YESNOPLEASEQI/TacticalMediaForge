    ...
    def chat(
        ...
        **kwargs
        ...
        try:
            ...
            raise RuntimeError(
                f"DashScope QwenVLClient error: {sanitize_error_message(e)}"
            ) from e
...
if __name__ == "__main__":
    ...
    text = "请描述这张图片的内容"
    print(f"\n[多模态] Prompt length: {len(text)} chars")
    print(f"  图片: {redact_path_for_log(img_path)}")
    ...
    for model in MODELS:
        ...
        try:
            ...
            if result:
                print(f"✓ 返回结果 ({elapsed:.1f}s, {len(str(result))} chars)")
            else:
