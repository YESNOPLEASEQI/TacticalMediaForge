    ...
    def generate_image(self,
        ...
        if is_seedream:
            ...
            try:
                ...
                logging.error(
                    f"Seedream generation failed: {sanitize_error_message(e)}"
                )
            ...
            try:
                ...
                if path and os.path.exists(path):
                    ...
                    logging.error(
                        "GPT/Sora returned invalid path or download failed: "
                        f"{redact_path_for_log(path)}"
                    )

            except Exception as e:
                logging.error(
                    f"GPT/Sora generation failed: {sanitize_error_message(e)}"
                )
            ...
            try:
                ...
                logging.error(
                    f"DashScope generation failed: {sanitize_error_message(e)}"
                )
                raise RuntimeError(
                    f"DashScope generation failed: {sanitize_error_message(e)}"
                ) from e
