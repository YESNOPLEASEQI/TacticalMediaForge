    ...
    def _encode_image_to_base64(self, image_path: str) -> str:
        ...
        try:
            ...
            print(
                ...
                f"{redact_path_for_log(image_path)}: {sanitize_error_message(e)}"
            )
            return image_path
        ...
        while attempts < self.max_attempts:
            try:
                ...
                print(
                    ...
                    f"{sanitize_error_message(e)}. Retrying in 10 seconds."
                )
                time.sleep(10)
...
if __name__ == "__main__":
    ...
    for model in MODELS:
        print(f"\nTesting model: {model}")
        print(f"Prompt length: {len(img_prompt)} chars")
        print(f"Image path: {redact_path_for_log(img_path)}")
        ...
        try:
            ...
            print(f"✓ 生成成功 ({elapsed:.1f}s): {redact_path_for_log(path)}")
        except Exception as e:
            elapsed = time.time() - t0
            print(f"✗ 失败 ({elapsed:.1f}s): {sanitize_error_message(e)}")
