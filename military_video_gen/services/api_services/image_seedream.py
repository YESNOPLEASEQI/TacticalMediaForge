...
class SeedreamClient:
    ...
    def __init__(
        ...
        client_kwargs = {
            ...
            "timeout": timeout,
            # Images API submissions are billable and non-idempotent.
            "max_retries": 0,
        }
        if self.local_proxy:
    ...
    def _download_image(self, url: str, session_id: str, idx: int) -> Optional[str]:
        ...
        try:
            ...
            logging.error(
                ...
                f"{redact_url_for_log(url)}: {sanitize_error_message(e)}"
            )
            return None
...
if __name__ == "__main__":
    ...
    print(f"\n[测试1: 文生图] Prompt length: {len(prompt)} chars")
    t0 = time.time()
    try:
        ...
        if paths:
            print(
                f"✓ 生成 {len(paths)} 张图片 ({elapsed:.1f}s): "
                f"{[redact_path_for_log(path) for path in paths]}"
            )
        else:
            print(f"✗ 返回空列表 ({elapsed:.1f}s)")
    except Exception as e:
        print(f"✗ 图片生成失败: {sanitize_error_message(e)}")
    ...
    if os.path.exists(ref_image_path):
        ...
        print(f"\n[测试2: 图生图] Prompt length: {len(prompt_i2i)} chars")
        print(f"  参考图: {redact_path_for_log(ref_image_path)}")
        t0 = time.time()
        try:
            paths = client.generate_image(
                prompt=prompt_i2i,
            ...
            print(f"✗ 图生图失败: {sanitize_error_message(e)}")
    else:
        print(
            "\n[测试2: 图生图] ✗ 参考图不存在: "
            f"{redact_path_for_log(ref_image_path)}"
        )
