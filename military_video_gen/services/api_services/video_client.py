...
class VideoClient:
    ...
    def generate_video(
        ...
        if Config.PRINT_MODEL_INPUT:
            ...
            print(f"Prompt length: {len(prompt or '')} chars")
            if image_path and str(image_path).startswith("data:"):
                print("Image: [Base64图片]")
            else:
                print(f"Image: {redact_path_for_log(image_path)}")
            print(f"Model: {model}")
            ...
            if last_image_path:
                print(f"Last Image: {redact_path_for_log(last_image_path)}")
            if first_clip_path:
                print(f"First Clip: {redact_path_for_log(first_clip_path)}")
            if reference_image_path:
                print(f"Reference Image: {redact_path_for_log(reference_image_path)}")
            if reference_image_paths:
                print(f"Reference Images: {len(reference_image_paths)} file(s)")
            if reference_video_paths:
                print(f"Reference Videos: {len(reference_video_paths)} file(s)")
            if reference_audio_path:
                print(f"Reference Audio: {redact_path_for_log(reference_audio_path)}")
            if audio_path:
                print(f"Audio: {redact_path_for_log(audio_path)}")
            if negative_prompt:
                print(f"Negative Prompt length: {len(negative_prompt)} chars")
            if sound:
                print(f"Sound: {sound}")
            print(f"Save: {redact_path_for_log(save_path)}")
            print("-" * 30)
    ...
    def cancel_active_operations(self) -> None:
        """Forward cooperative cancellation to initialized provider clients."""
        for client in (
            self._dashscope_client,
            self._kling_client,
            self._seedance_client,
        ):
            cancel = getattr(client, "cancel_active_operations", None)
            if callable(cancel):
                cancel()
