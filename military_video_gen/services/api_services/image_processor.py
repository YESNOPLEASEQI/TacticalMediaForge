...
class ImageProcessor:
    ...
    def download_image(self, image_url, save_path, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    with open(save_path, 'wb') as file:
                        for chunk in response.iter_content(chunk_size=8192):
                            if chunk:
                                file.write(chunk)
                    logging.info(f"图片下载成功: {redact_path_for_log(save_path)}")
                    return True
                else:
                ...
                logging.warning(
                    f"SSL错误 (尝试 {attempt + 1}/{max_retries}): "
                    f"{sanitize_error_message(e)[:100]}"
                )
                ...
                logging.warning(
                    f"超时错误 (尝试 {attempt + 1}/{max_retries}): "
                ...
                logging.warning(
                    f"下载错误 (尝试 {attempt + 1}/{max_retries}): "
                    f"{sanitize_error_message(e)}"
                )
                if attempt < max_retries - 1:
                    time.sleep((attempt + 1) * 2)
    ...
    def upload_file_to_oss(self, policy_data: dict, file_path: str) -> str:
        ...
        with open(file_path, 'rb') as file:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Failed to upload file: {sanitize_error_message(response.text)}"
                )
    ...
    def upload(self, file_path: str) -> str:
        ...
        if not os.path.exists(file_path):
            raise FileNotFoundError(
                f"文件不存在: {redact_path_for_log(file_path)}"
            )
        ...
        logging.info(f"文件上传成功: {redact_path_for_log(file_path)}")
        logging.info(f"  OSS URL: {redact_url_for_log(oss_url)}")
    ...
    def collage_images(self, image_paths, output_path):
        ...
        for p in image_paths:
            try:
                ...
                logging.error(
                    f"Cannot open image {redact_path_for_log(p)}: "
                    f"{sanitize_error_message(e)}"
                )
