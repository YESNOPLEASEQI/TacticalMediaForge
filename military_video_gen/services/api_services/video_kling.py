...
def _build_session(max_retries: int = 3) -> requests.Session:
    ...
    retry = Retry(
        ...
        # Provider task creation is non-idempotent. Never let urllib3 replay
        # POST after an ambiguous transport or gateway failure.
...
class KlingVideoClient:
    ...
    def _encode_image(image_path: str, quality: int = 85) -> str:
        ...
        try:
            ...
            logger.warning(
                "图片压缩失败 "
                f"({redact_path_for_log(image_path)})，使用原始文件: "
    ...
    def _submit_task(
        ...
        if not resp.ok:
            try:
                ...
                err_body = resp.text
            logger.error(
                f"KlingVideoClient: HTTP {resp.status_code}: "
                f"{sanitize_error_message(err_body)}"
            )
    ...
    def _download_video(video_url: str, save_path: str) -> None:
        ...
        with open(save_path, "wb") as f:
            for chunk in resp.iter_content(chunk_size=8192):
                if chunk:
                    f.write(chunk)
        logger.info(f"KlingVideoClient: video saved: {redact_path_for_log(save_path)}")
