...
class LLMService:
    ...
    Usage:
        ...
        # With parameters
        answer = await military_video_gen.llm(
    ...
    def _create_client(
        ...
        client_kwargs = {
            ...
            "timeout": 90.0,
            "max_retries": 2,
            # Text generation must not silently depend on a desktop proxy inherited
            # from the shell that launched the API. Provider-specific proxy support
            # is configured explicitly elsewhere.
    ...
    async def __call__(
        ...
        **kwargs
        ...
        Examples:
            ...
            review = await military_video_gen.llm(
        ...
        safe_base_url = redact_url_for_log(str(client.base_url))
        logger.debug(
            f"LLM call: model={final_model}, base_url={safe_base_url}, "
        ...
        try:
            if response_type is not None:
                ...
                if not result or not result.strip():
                    logger.warning(
                        f"LLM returned empty text content (model={final_model}, "
            ...
            logger.error(
                f"LLM call error (model={final_model}, base_url={safe_base_url}): "
    ...
    async def generate_structured(
        ...
        **kwargs,
        ...
        """Generate typed output while preserving each supplied message role."""
        if not messages:
    ...
    async def _call_with_structured_output(
        ...
        **kwargs
        ...
        Args:
            ...
            model: Model name
            messages: System/user/assistant messages for the provider
        ...
        """
        # Build JSON schema instruction and append only to the last user message.
        ...
        user_indexes = [
            index
            for index, message in enumerate(enhanced_messages)
            if message.get("role") == "user"
        ]
        if not user_indexes:
            raise ValueError("structured messages require a user message")
        user_index = user_indexes[-1]
        enhanced_messages[user_index]["content"] = (
            f"{enhanced_messages[user_index].get('content', '')}"
        ...
        if not content or not content.strip():
            logger.warning(
                f"LLM returned empty structured-output content (model={model}, "
                f"base_url={redact_url_for_log(str(client.base_url))})"
            )
...
Output ONLY the JSON object, nothing else."""
            ...
            logger.warning(
                f"Failed to generate JSON schema: {sanitize_error_message(e)}"
            )
...
You MUST respond with ONLY a valid JSON object (no markdown, no extra text)."""
    ...
    def active(self) -> str:
        ...
        Example:
            print(f"Using model: {military_video_gen.llm.active}")
        """
    ...
    def __repr__(self) -> str:
        ...
        return f"<LLMService model={model!r} base_url={redact_url_for_log(str(base_url))!r}>"
