...
class PersistenceService:
    ...
    def __init__(self, output_dir: str = "output", database_sync=None):
        """
        ...
        self.database_sync = database_sync or HistoryDatabaseSync()
    ...
    def get_task_dir(self, task_id: str) -> Path:
        """Get a contained task directory path.

        Task IDs are opaque identifiers, never path fragments.  Keeping this
        check at the persistence boundary protects every read/write/delete
        caller, including the history API.
        """
        if (
            ...
            raise ValueError("task_id must be a single safe identifier")
        root = self.output_dir.resolve()
        candidate = (root / task_id).resolve()
        if candidate.parent != root:
            raise ValueError("task_id resolves outside the task output directory")
        return candidate
    ...
    async def save_task_metadata(
        ...
        try:
            ...
            await self._sync_database(task_id, metadata)
            
        except Exception as e:
            logger.error(
                f"Failed to save task metadata {task_id}: {sanitize_error_message(e)}"
            )
            raise
    ...
    async def load_task_metadata(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to load task metadata {task_id}: {sanitize_error_message(e)}"
            )
            return None
    ...
    async def update_task_status(
        ...
        try:
            ...
            logger.error(
                f"Failed to update task status {task_id}: {sanitize_error_message(e)}"
            )
    ...
    async def save_storyboard(
        ...
        try:
            ...
            metadata = await self.load_task_metadata(task_id)
            if metadata:
                await self._sync_database(task_id, metadata, storyboard_dict)
            
        except Exception as e:
            logger.error(
                f"Failed to save storyboard {task_id}: {sanitize_error_message(e)}"
            )
            raise
    ...
    async def _sync_database(
        ...
        """Best-effort relational mirror; JSON remains the compatibility source."""
        try:
            await self.database_sync.sync_task(task_id, metadata, storyboard)
        except Exception as exc:
            logger.error(
                "Database dual-write failed for task "
                f"{task_id}: {sanitize_error_message(exc)}"
            )
    ...
    async def load_storyboard(self, task_id: str) -> Optional[Storyboard]:
        ...
        try:
            ...
            logger.error(
                f"Failed to load storyboard {task_id}: {sanitize_error_message(e)}"
            )
            return None
    ...
    async def list_tasks(
        ...
        try:
            ...
            logger.error(f"Failed to list tasks: {sanitize_error_message(e)}")
            return []
    ...
    def _frame_to_dict(self, frame: StoryboardFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_duration": frame.estimated_duration,
            "research_metadata": frame.research_metadata,
    ...
    def _dict_to_frame(self, data: Dict[str, Any]) -> StoryboardFrame:
        ...
        return StoryboardFrame(
            ...
            research_metadata=data.get("research_metadata") or {},
    ...
    def _load_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to load index: {sanitize_error_message(e)}")
    ...
    def _save_index(self, index_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Failed to save index: {sanitize_error_message(e)}")
    ...
    async def delete_task(self, task_id: str) -> bool:
        ...
        try:
            ...
            if task_dir.exists():
                ...
                logger.info(
                    f"Deleted task directory: {redact_path_for_log(task_dir)}"
                )
            ...
            logger.error(
                f"Failed to delete task {task_id}: {sanitize_error_message(e)}"
            )
            return False
