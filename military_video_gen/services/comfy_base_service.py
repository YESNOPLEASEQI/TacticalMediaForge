...
class ComfyBaseService:
    ...
    def _parse_workflow_file(self, file_path: Path, source: str) -> Dict[str, Any]:
        ...
        workflow_info = {
            ...
            "source": source,
            "path": f"workflows/{source}/{file_path.name}",
            "_internal_path": str(file_path),
    ...
    def _prepare_comfykit_config(
        ...
        logger.debug(
            "ComfyKit request configured: source={}, api_key_configured={}, "
            "runninghub_key_configured={}",
            kit_config.get("source"),
            bool(kit_config.get("api_key")),
            bool(kit_config.get("runninghub_api_key")),
        )
        return kit_config
